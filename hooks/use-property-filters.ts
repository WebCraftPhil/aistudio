...
import { ALL_STATUSES, ALL_TAGS, type PropertyStatus, type PropertyTag } from "@/lib/mock/properties"
...
export function usePropertyFilters() {
  ...
  const propertyFilters = {
    ...
    sort: sortColumn && sortDirection ? [sortColumn, sortDirection] as [SortableColumn, SortDirection] : undefined,
  }

  // Helper to check if any filters are active
  const hasActiveFilters = !!(filters.q || filters.status || (filters.tags && filters.tags.length > 0))
